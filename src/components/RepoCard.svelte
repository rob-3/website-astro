...
<a href={html_url} class="no-underline">
  <div
    bind:this={ref}
    in:fade
    class="group flex h-full flex-col rounded-sm border border-slate-300 bg-slate-100 p-4 no-underline drop-shadow-lg transition-transform hover:translate-x-1 hover:translate-y-1 hover:cursor-pointer"
  >
    <div>
      <svg
        class="inline-block"
        ...
        /></svg
      >
      <span class="font-medium text-blue-700 group-hover:underline">{name}</span
      >
    </div>
    {#if fork}
      <div class="text-xs text-[#586069]">
        Forked from <a
          ...
          href={source.html_url}>{source.full_name}</a
        >
      </div>
    {/if}
